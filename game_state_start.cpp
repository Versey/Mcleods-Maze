    ...
    while(this->game->window.pollEvent(event))
    {
        switch(event.type)
        {
            ...
            case sf::Event::Resized:
            {
                ...
                //this->game->background.setPosition(0,0);

                sf::Vector2f pos2 = this->game->background.getPosition();
                std::cout << "start background pos x:" << pos2.x << " pos y: " << pos2.y << std::endl;
...
GameStateStart::GameStateStart(Game* game)
{
	...
	this->guiSystem.emplace("menu", Gui(sf::Vector2f(140, 32), 5, false, game->stylesheets.at("button"),
		{ std::make_pair("START", "load_game") }));

	this->guiSystem.at("menu").setPosition(pos);
	this->guiSystem.at("menu").setOrigin(92, 32*1/2);
