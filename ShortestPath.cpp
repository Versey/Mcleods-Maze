...
ShortestPath::ShortestPath(Maze maze) : maze(maze)
{
	...
	// get the route
	shortestRoute = findPath(xStart, yStart, xEnd, yEnd);
	if (shortestRoute == "") cout << "An empty route generated!" << endl;
	...
	if (shortestRoute.length()>0)
	{
		...
		for (unsigned int i = 0; i<shortestRoute.length(); i++)
		{
			directionAsC = shortestRoute.at(i);
